...
class OMXPlayer(object):
    ...
    def __init__(self, mediafile, args=None, start_playback=False):
        ...
        self.audio = dict()
        # Get file properties
        #print('Process:', process_str)
        #file_props = self._FILEPROP_REXP.match(process_str).groups()
        #(self.audio['streams'], self.video['streams'],
        # self.chapters, self.subtitles) = [int(x) for x in file_props]
        # Get video properties
        process_str = self._process.readline()
        video_props = self._VIDEOPROP_REXP.match(process_str).groups()
